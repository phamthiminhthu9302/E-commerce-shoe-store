...
<div id="logreg-forms">
            <form class="form-signin" action="forgotPassword" method="post">
                <h1 class="h3 mb-3 font-weight-normal" style="text-align: center"> Forgot Password</h1>
                 <c:if test="${mess!=null }">
                <div class="alert alert-success" role="alert" id="sucess">${mess}</div>
					  </c:if>
                <c:if test="${error!=null }">
                <div class="alert alert-danger" role="alert" >${error}</div>
               </c:if>
                <label for="username">Username</label>
                <input name="username" type="text" value="${account.user}" id="username" class="form-control" placeholder="Username" required="" autofocus="">
                <label for="email">Email</label>
                <input name="email" type="email" id="email" value="${account.email}" class="form-control" placeholder="Email" required="" autofocus="">
