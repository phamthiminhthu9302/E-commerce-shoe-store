							...
							<table class="table table-hover text-nowrap">
								<thead>
									<tr>
										...
										<th scope="col" class="border-1 bg-light ">Mã hóa đơn</th>
										<th scope="col" class="border-1 bg-light ">Account khách hàng</th>
										<th scope="col" class="border-1 bg-light ">Account nhân viên</th>
										<th scope="col" class="border-1 bg-light ">Tổng giá ($)</th>
										<th scope="col" class="border-1 bg-light ">Ngày Xuất</th>
								...
								<tbody id="content">
									...
									for (OrderDetail o : ListOrder) {
										...
										if (i.getmaHD() == o.getOrderID() && mahd != i.getmaHD()) {
											account = dao.FindAccountByID(i.getAccountID());
											mahd = i.getmaHD();
									%>
									<tr <%if (o.getOrderID() > 0) {%>
										id="<%=o.getOrderID()%>" <%}%>>
										<td><%=i.getmaHD()%></td>
										<%
										for (Account t : listAllAccount) {
											if (t.getId() == o.getAccountID()) {
										%>
										<td><%=t.getUser()%></td>
										<%
										}
					...
					.ajax({
						...
						success : function(responseData) {
							if(responseData){
								...
								document.getElementById('content').style.display = "table-row-group";
							}else{
